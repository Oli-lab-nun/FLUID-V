...
class MDVLModelV6(PreTrainedModel, GenerationMixin):
    """
    Stage-2 dynamic-K head training wrapper on top of a frozen V6 teacher.

    This file keeps the V6 helper functions so the trained recurrent model can
    act as a label generator. During training, the backbone is frozen and only a
    small diffusion_k_head is optimized. Labels are produced online by forcing
    max-K probe blocks, running the frozen V6 refinement path, and measuring how
    far each trigger can diffuse before the average mask loss becomes too high.
    """
    ...
    def __init__(
        ...
        k_head_loss_threshold: Optional[float] = None,
        **kwargs,
    ):
        ...
        self.diffusion_k_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(self.hidden_size // 4, self.max_k),
        )
        self.k_head_probe_ratio = 0.3
        self.k_head_loss_threshold = (
            float(k_head_loss_threshold) if k_head_loss_threshold is not None else 2.8
        )
        self.k_head_label_sigma = 1.5
        self._k_head_train_sanity_logged = False
        self._k_head_no_signal_warn_count = 0

        self._enable_k_head_training_only()
        self._assert_k_head_trainable()

        logger.info(
            "[OpenPangu-VL][MDVL-V6-Head] single-branch k-head training enabled; "
            "probe=%.2f, k_loss_thr=%.2f, sigma=%.2f.",
            self.k_head_probe_ratio,
            self.k_head_loss_threshold,
            self.k_head_label_sigma,
        )
        logger.info(
            "[OpenPangu-VL][MDVL-V6-Head] trainable parameter count after local freeze: %d",
    ...
    def _enable_k_head_training_only(self) -> None:
        for param in self.parameters():
            param.requires_grad = False
        for param in self.diffusion_k_head.parameters():
            param.requires_grad = True

    def _count_trainable_parameters(self) -> int:
        return sum(param.numel() for param in self.parameters() if param.requires_grad)

    def _assert_k_head_trainable(self) -> None:
        trainable_names = [name for name, param in self.named_parameters() if param.requires_grad]
        if not trainable_names:
            raise RuntimeError("K-head 训练配置错误：当前没有任何可训练参数。")

        invalid_names = [name for name in trainable_names if not name.startswith("diffusion_k_head.")]
        if invalid_names:
            raise RuntimeError(
                "K-head 训练配置错误：检测到非 diffusion_k_head 参数被错误地设为可训练，"
                f"例如: {invalid_names[:8]}"
            )
    ...
    def _compute_per_token_loss(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        return token_losses.view_as(labels)
    ...
    def _build_gaussian_soft_targets(
        ...
        class_indices = torch.arange(num_classes, device=target_indices.device).unsqueeze(0)
        class_indices = class_indices.expand(target_indices.size(0), -1)
        target_centers = target_indices.unsqueeze(1)
        dist_sq = (class_indices - target_centers) ** 2
        soft_targets = torch.exp(-dist_sq / (2.0 * float(sigma) ** 2))
        return soft_targets / soft_targets.sum(dim=1, keepdim=True).clamp_min(1e-8)

    def _prepare_batch_for_k_head_training(
        self,
        ...
        """
        Build a probe batch for dynamic-K head training.

        Different from normal V6 training:
        1. Once a response column is selected, it is expanded to exactly max_k slots.
        2. No random restore is applied; the frozen V6 teacher should face the full block.
        3. The resulting per-mask losses are used to derive an online target K.
        """
        device = input_ids.device
        batch_size, seq_len = input_ids.shape
        if batch_size != 1:
            raise NotImplementedError("MDVLModelV6 dynamic-K head training当前只支持 per-device batch_size=1。")
        ...
        remask_block_mask = is_response_col & (rand_probs < float(self.k_head_probe_ratio))

        k_counts = torch.zeros(seq_len, dtype=torch.long, device=device)
        full_k = torch.tensor(self.max_k, dtype=torch.long, device=device)
        k_counts = torch.where(remask_block_mask, full_k, k_counts)

        if is_response_col.any() and not bool((k_counts > 0).any().item()):
            first_valid = torch.nonzero(is_response_col, as_tuple=False)[0, 0]
            k_counts[first_valid] = self.max_k
        ...
        meta = {
            ...
            "k_counts": k_counts,
            "effective_k": torch.tensor(self.max_k, device=device),
    ...
    def _calculate_cutoff_k(
        self,
        token_losses: torch.Tensor,
        is_trigger: torch.Tensor,
        valid_mask_positions: torch.Tensor,
        max_k: int,
        threshold: float,
    ) -> torch.Tensor:
        optimal_ks = torch.ones_like(token_losses, dtype=torch.float32)
        cumulative_loss = torch.zeros_like(token_losses, dtype=torch.float32)

        for k in range(1, max_k + 1):
            shifted_loss = torch.roll(token_losses, shifts=-k, dims=1)
            cumulative_loss = cumulative_loss + shifted_loss
            avg_loss = cumulative_loss / float(k)

            shifted_valid_mask = torch.roll(valid_mask_positions, shifts=-k, dims=1)
            update_mask = is_trigger & shifted_valid_mask & (avg_loss < float(threshold))
            optimal_ks = torch.where(
                update_mask,
                torch.tensor(float(k), device=token_losses.device, dtype=optimal_ks.dtype),
                optimal_ks,
            )
    ...
    def _update_middle_state(
        ...
        norm_weight = next(self.middle_state_norm.parameters())
        next_state = self.middle_state_norm(denoise_blend.to(device=norm_weight.device, dtype=norm_weight.dtype)).to(
            device=compute_device,
            dtype=compute_dtype,
        )
    ...
    def _run_k_head_training_step(
        ...
        *,
        ...
        (
            ...
            _new_attention_mask_2d,
            meta,
        ) = self._prepare_batch_for_k_head_training(input_ids, labels, attention_mask)

        if not self._k_head_train_sanity_logged:
            self._assert_k_head_trainable()
            logger.info(
                "[OpenPangu-VL][MDVL-V6-Head] k-head train sanity checked, trainable params=%d",
                self._count_trainable_parameters(),
            )
            self._k_head_train_sanity_logged = True
        ...
        with torch.no_grad():
            teacher_outputs = self._model_core_forward(
                inputs_embeds=base_embeddings,
                attention_mask=new_attention_mask_4d,
                position_ids=position_ids,
            )
            teacher_hidden = teacher_outputs.last_hidden_state
            teacher_logits_all = self.lm_head(teacher_hidden)
            if has_vision_inputs:
                teacher_logits_all = self._mask_mm_forbidden_logits(teacher_logits_all)

            token_losses = self._compute_per_token_loss(teacher_logits_all, new_labels)
            is_mask_bool = meta["is_mask"].unsqueeze(0).expand_as(new_labels)
            valid_mask_positions = is_mask_bool & meta["valid_target"].unsqueeze(0).expand_as(new_labels)
            shifted_mask = torch.roll(is_mask_bool, shifts=-1, dims=1)
            shifted_mask[:, -1] = False
            is_trigger = (~is_mask_bool) & shifted_mask
            target_ks = self._calculate_cutoff_k(
                token_losses=token_losses,
                is_trigger=is_trigger,
                valid_mask_positions=valid_mask_positions,
                max_k=self.max_k,
                threshold=self.k_head_loss_threshold,
            )

        if bool(is_trigger.any().item()):
            trigger_states = teacher_hidden[is_trigger]
            pred_logits = self.diffusion_k_head(trigger_states)
            target_indices = (target_ks.long() - 1).clamp(min=0, max=self.max_k - 1)
            soft_targets = self._build_gaussian_soft_targets(
                target_indices=target_indices,
                num_classes=self.max_k,
                sigma=self.k_head_label_sigma,
            ).to(dtype=pred_logits.dtype)
            log_probs = F.log_softmax(pred_logits, dim=-1)
            loss_head = -(soft_targets * log_probs).sum(dim=-1).mean()
        else:
            if self._k_head_no_signal_warn_count < 8:
                logger.warning(
                    "[OpenPangu-VL][MDVL-V6-Head] no valid trigger found in current batch, returning zero loss. "
                    "num_blocks=%d, valid_targets=%d",
                    int(meta["num_blocks"].item()),
                    int(meta["valid_target"].sum().item()),
                )
                self._k_head_no_signal_warn_count += 1
            dummy_pred = self.diffusion_k_head(teacher_hidden[:, :1, :].reshape(-1, teacher_hidden.size(-1)))
            loss_head = dummy_pred.sum() * 0.0

        loss_head = self._ensure_loss_has_grad(loss_head)
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self._run_k_head_training_step(
            input_ids=input_ids,
            labels=labels,
            attention_mask=attention_mask,
            pixel_values=pixel_values,
    ...
    def _get_wrapper_state_dict(self) -> Dict[str, torch.Tensor]:
        """
        Save wrapper-side weights that do not belong to the underlying base model.

        This is important for stage-2 training:
        1. `diffusion_k_head.*` is newly introduced in this file.
        2. V6 itself already contains wrapper parameters (e.g. `middle_state_norm.*`)
           that are randomly initialized outside the base model and then trained in
           stage-1. If we only save the K-head, reloading will silently fall back to
           fresh random wrapper weights and break teacher consistency.
        """
        wrapper_state = {}
        ...
        for key, value in full_state.items():
            ...
            if key.startswith("lm_head."):
                continue
            wrapper_state[key] = value.detach().cpu()
    ...
    def save_pretrained(self, save_directory: str, **kwargs):
        ...
        head_state_dict = self._get_wrapper_state_dict()
        output_file = os.path.join(save_directory, "model_head.bin")
        torch.save(head_state_dict, output_file)
        metadata = {
            "wrapper_variant": "nothink_v6_head",
            "teacher_adapter_path": getattr(self, "teacher_adapter_path", None),
            "saved_wrapper_keys": sorted(head_state_dict.keys()),
            "max_k": int(self.max_k),
            "k_head_probe_ratio": float(self.k_head_probe_ratio),
            "k_head_loss_threshold": float(self.k_head_loss_threshold),
            "k_head_label_sigma": float(self.k_head_label_sigma),
        }
        metadata_file = os.path.join(save_directory, "model_head_meta.json")
        with open(metadata_file, "w", encoding="utf-8") as f:
            json.dump(metadata, f, ensure_ascii=False, indent=2)
        logger.info(
            "[MDVL-V6-Head] 已保存 wrapper 权重至: %s, meta至: %s (keys=%s)",
            output_file,
            metadata_file,
            sorted(head_state_dict.keys()),
        )

    @staticmethod
    def load_model_for_inference(model, head_checkpoint_path: str):
        ...
        if os.path.exists(head_weight_path):
            ...
            logger.info(
                "[MDVL-V6-Head] wrapper/head 加载成功，missing=%d, unexpected=%d, loaded_keys=%s",
                len(missing_keys),
                len(unexpected_keys),
                sorted(state_dict.keys()),
            )
            meta_path = os.path.join(head_checkpoint_path, "model_head_meta.json")
            if os.path.exists(meta_path):
                try:
                    with open(meta_path, "r", encoding="utf-8") as f:
                        metadata = json.load(f)
                    teacher_adapter_path = metadata.get("teacher_adapter_path", None)
                    if teacher_adapter_path is not None:
                        setattr(model, "teacher_adapter_path", teacher_adapter_path)
                    sigma = metadata.get("k_head_label_sigma", None)
                    if sigma is not None:
                        model.k_head_label_sigma = float(sigma)
                except Exception as exc:
                    logger.warning("[MDVL-V6-Head] 读取 meta 失败: %s", exc)
        else:
            logger.warning("[MDVL-V6-Head] 未找到 Head 权重文件: %s", head_weight_path)
        return model
    ...
    def generate_dynamic_kv(
        ...
        disable_middle_state: bool = False,
        ...
        max_block_size = max(1, min(int(block_size), self.max_k))
        has_vision_inputs = pixel_values is not None or pixel_values_videos is not None
        finished = False
        ...
        predicted_block_sizes: List[int] = []
        ...
        def _predict_block_size(
            trigger_hidden: torch.Tensor,
            remaining_budget: int,
        ) -> int:
            if forced_block_size is not None:
                pred_k = int(forced_block_size)
            else:
                head_weight = next(self.diffusion_k_head.parameters())
                head_input = trigger_hidden.reshape(-1, trigger_hidden.size(-1)).to(
                    device=head_weight.device,
                    dtype=head_weight.dtype,
                )
                pred_logits = self.diffusion_k_head(head_input).float()
                pred_index = int(torch.argmax(pred_logits, dim=-1).item())
                pred_k = pred_index + 1
            pred_k = max(1, min(pred_k, max_block_size, remaining_budget))
            return pred_k
        ...
        while (current_ids.shape[1] - initial_len) < max_new_tokens:
            ...
            cur_block = _predict_block_size(current_prefix_hidden, remaining_budget)
            predicted_block_sizes.append(int(cur_block))
            ...
            block_decode_lens: List[int] = []
            block_conf_mins: List[float] = []
            block_conf_means: List[float] = []
            ...
            while unresolved_len > 0:
                ...
                diffusion_outputs = _run_generate_segment(
                    ...
                    middle_state=None if disable_middle_state else current_middle_state,
                ...
                decoded_conf = token_conf[:, :decode_len].detach().float()
                block_decode_lens.append(int(decode_len))
                block_conf_mins.append(float(decoded_conf.min().item()) if decoded_conf.numel() else 0.0)
                block_conf_means.append(float(decoded_conf.mean().item()) if decoded_conf.numel() else 0.0)
                ...
                if disable_middle_state:
                    diffusion_state = current_middle_state
                else:
            ...
            if block_decode_lens:
                horizon_events.append(
                    {
                        "predicted_k": int(cur_block),
                        "decoded_total": int(sum(block_decode_lens)),
                        "num_diffusion_steps": int(len(block_decode_lens)),
                        "decode_lens": [int(x) for x in block_decode_lens],
                        "first_decode_len": int(block_decode_lens[0]),
                        "min_conf": float(min(block_conf_mins)) if block_conf_mins else 0.0,
                        "mean_conf": float(sum(block_conf_means) / max(1, len(block_conf_means))),
                    }
                )
        ...
        stats = {
            ...
            "predicted_block_sizes": predicted_block_sizes,
            "horizon_events": horizon_events,
            "disable_middle_state": bool(disable_middle_state),
            "mean_predicted_block_size": float(sum(predicted_block_sizes)) / float(max(1, len(predicted_block_sizes))),
