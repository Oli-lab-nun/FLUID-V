...
class ModelArguments(
    ...
    max_train_blocks: Optional[int] = field(
        default=None,
        metadata={"help": "Maximum number of sampled FLUID-V blocks per training sample."},
    )

    md_model_variant: str = field(
        default="auto",
        metadata={"help": "Custom MD wrapper variant, e.g. agent_edit_resync."},
    )

    md_train_stage: str = field(
        default="stage1",
        metadata={"help": "MD training stage for wrappers with staged mask schedules, e.g. stage1 or stage2."},
    )

    md_mask_token: str = field(
        default="<mask>",
        metadata={"help": "Tokenizer token used as the MD mask token."},
    )

    md_mask_insert_prob: float = field(
        default=0.25,
        metadata={"help": "Probability of inserting mask tokens at response positions for MDModel."},
    )

    md_restore_ratio_max: float = field(
        default=0.5,
        metadata={"help": "Upper bound of the random mask restoration ratio for MDModel."},
    )

    md_noise_prob: float = field(
        default=0.05,
        metadata={"help": "Probability of restoring a masked position with a random non-special token."},
    )

    openpangu_vl_variant: str = field(
        default="think",
        metadata={"help": "MD wrapper variant for OpenPangu-VL, e.g. think or nothink."},
    )
