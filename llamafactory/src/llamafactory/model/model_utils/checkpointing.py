...
def _enable_llada_activation_checkpointing(model: "PreTrainedModel") -> bool:
    if str(getattr(getattr(model, "config", None), "model_type", "")).lower() != "llada":
        return False

    llada_core = getattr(model, "model", None)
    if llada_core is None or not hasattr(llada_core, "set_activation_checkpointing"):
        return False

    llada_module = importlib.import_module(llada_core.__class__.__module__)
    strategy_cls = getattr(llada_module, "ActivationCheckpointingStrategy", None)
    if strategy_cls is None or not hasattr(strategy_cls, "whole_layer"):
        return False

    llada_core.set_activation_checkpointing(strategy_cls.whole_layer)
    setattr(model.config, "use_cache", False)
    logger.info_rank0("LLaDA activation checkpointing enabled.")
    return True
...
def prepare_model_for_training(model: "PreTrainedModel", model_args: "ModelArguments") -> None:
    ...
    if not model_args.disable_gradient_checkpointing:
        if _enable_llada_activation_checkpointing(model):
            pass
        elif not getattr(model, "supports_gradient_checkpointing", False):
