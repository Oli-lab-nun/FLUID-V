...
except ImportError:  # transformers>=5.2 may remove this alias
...
def _patch_transformers_none_tp_plan() -> None:
    import transformers.modeling_utils as modeling_utils

    original_warmup = modeling_utils.caching_allocator_warmup
    if getattr(original_warmup, "_llamafactory_none_tp_plan_patch", False):
        return
    ...
    patched_warmup._llamafactory_none_tp_plan_patch = True
...
def load_tokenizer(model_args: "ModelArguments") -> "TokenizerModule":
    ...
    if "glm" in str(model_args.model_name_or_path).lower():
        tokenizer_extra_kwargs["fix_mistral_regex"] = True
    try:
...
def load_model(
    ...
    _patch_transformers_none_tp_plan()
    ...
    if model is None and not lazy_load:
        ...
        if getattr(config, "model_type", None) == "internvl_chat":
            init_kwargs["use_flash_attn"] = False
        ...
        if model_args.mixture_of_depths == "load":
            ...
            if AutoModelForVision2Seq is not None and type(config) in AutoModelForVision2Seq._model_mapping.keys():
                load_class = AutoModelForVision2Seq  # image-text
            ...
            elif str(getattr(config, "model_type", "")).lower() == "dream":
    ...
    #     import sys
    #     sys.path.append(os.path.join("/path/to/project"))
    ...
    if model_args.IS_MDModel:
        ...
        from model import get_md_model

        model = get_md_model(
            base_model=model,
            k_masks=model_args.k_masks,
            is_trainable=is_trainable,
            md_model_variant=model_args.md_model_variant,
            mask_token=model_args.md_mask_token,
            mask_insert_prob=model_args.md_mask_insert_prob,
            restore_ratio_max=model_args.md_restore_ratio_max,
            noise_prob=model_args.md_noise_prob,
            md_train_stage=model_args.md_train_stage,
            openpangu_vl_variant=model_args.openpangu_vl_variant,
            max_train_blocks=model_args.max_train_blocks,
            lora_adapter_path=model_args.lora_adapter_path,
        )
        model.set_tokenizer(tokenizer)
    # Keep composite multimodal wrappers intact. Unwrapping to `language_model`
    # drops multimodal forward logic and may also lose generation_config.
    if hasattr(model, "language_model") and not hasattr(model.config, "text_config"):
        model = model.language_model
    if hasattr(config, "text_config") and not hasattr(model.config, "text_config"):
