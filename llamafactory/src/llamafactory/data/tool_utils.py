...
CALL_DSL_TOOL_PROMPT = """

You are a function-calling agent.

When a user request requires one or more available functions, output only CALL DSL blocks. If no function is suitable or a required argument is missing, output exactly NO_CALL.

CALL DSL format:
CALL <function_name>
<argument_name> = <json_literal>
END

Rules:
- Do not include explanations, markdown, code fences, or extra text in a tool-decision response.
- Use only function names and argument names from the available function schemas. Copy them exactly.
- Include every required argument. Omit optional arguments unless the user explicitly provides them.
- Every argument value must be a valid JSON literal. Strings and enum values use double quotes; numbers, booleans, and null use JSON syntax; arrays and objects use compact one-line JSON.
- Preserve user-provided string values exactly.
- For multiple calls, output multiple CALL blocks in execution order.
- After receiving a tool response, answer the user naturally unless another function call is required.
...
class CallDSLToolUtils(ToolUtils):
    r"""Line-based CALL DSL with JSON literals for argument values."""

    @override
    @staticmethod
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        return CALL_DSL_TOOL_PROMPT.format(tool_text=json.dumps(tools, ensure_ascii=False, indent=2))
    ...
    def function_formatter(functions: list["FunctionCall"]) -> str:
        ...
        for name, arguments in functions:
            ...
            if not isinstance(parsed_arguments, dict):
                raise ValueError(f"CALL DSL arguments for {name!r} must be an object.")
            lines = [f"CALL {name}"]
            lines.extend(
            ...
            lines.append("END")
            blocks.append("\n".join(lines))
        return "\n".join(blocks) if blocks else "NO_CALL"
            ...
            for raw_line in stripped.splitlines():
                ...
                if line.startswith("CALL "):
                    ...
                    calls.append(
                        FunctionCall(
                            current_name,
                            json.dumps(current_arguments, ensure_ascii=False, separators=(",", ":")),
                        )
                    )
                    current_name = None
                    current_arguments = {}
                    ...
                    if not key or key in current_arguments:
                        return content
                    current_arguments[key] = json.loads(raw_value)
        except json.JSONDecodeError:
            return content

        if current_name is not None or not calls:
            return content
        return calls


TOOLS = {
    "call_dsl": CallDSLToolUtils(),
