...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        batch_effective_k, batch_bucket_cutoff_len, batch_train_mode_ids = [], [], []
        for feature in features:
            images = feature.pop("images", None) or []
            videos = feature.pop("videos", None) or []
            audios = feature.pop("audios", None) or []
            batch_effective_k.append(feature.pop("effective_k", None))
            batch_bucket_cutoff_len.append(feature.pop("bucket_cutoff_len", None))
            train_mode_id = feature.pop("train_mode_id", None)
            if train_mode_id is None:
                train_mode = feature.pop("train_mode", None)
                train_mode_id = TRAIN_MODE_TO_ID.get(train_mode, TRAIN_MODE_TO_ID["dynamic"])
            batch_train_mode_ids.append(train_mode_id)
        ...
        if any(value is not None for value in batch_effective_k):
            features["effective_k"] = torch.tensor(batch_effective_k, dtype=torch.long)

        if any(value is not None for value in batch_bucket_cutoff_len):
            features["bucket_cutoff_len"] = torch.tensor(batch_bucket_cutoff_len, dtype=torch.long)

        features["train_mode_id"] = torch.tensor(batch_train_mode_ids, dtype=torch.long)
