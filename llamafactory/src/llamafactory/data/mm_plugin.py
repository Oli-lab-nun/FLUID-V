...
class MMPluginMixin:
    ...
    def _get_video_sample_indices(
        ...
        if video_fps <= 0:
            ...
            return np.linspace(0, total_frames - 1, video_maxlen).astype(np.int32)

        if total_frames == 0:  # infinite video
            return np.linspace(0, video_maxlen - 1, video_maxlen).astype(np.int32)

        sample_frames = math.floor(float(video_stream.duration * video_stream.time_base) * video_fps)
        sample_frames = max(1, min(total_frames, video_maxlen, sample_frames))
        return np.linspace(0, total_frames - 1, sample_frames).astype(np.int32)

    def _decode_sampled_video_frames(self, container: "InputContainer", video_stream: "Stream", sample_indices: list[int]) -> list["ImageObject"]:
        r"""Decode sampled frames while preserving repeated indices from fixed-frame sampling."""
        frames: list[ImageObject] = []
        index_counts = collections.Counter(int(index) for index in sample_indices)
        if not index_counts:
            return frames

        container.seek(0)
        for frame_idx, frame in enumerate(container.decode(video_stream)):
            repeat = index_counts.get(frame_idx, 0)
            if repeat <= 0:
                continue

            image = frame.to_image()
            frames.extend(image.copy() for _ in range(repeat))
    ...
    def _regularize_videos(self, videos: list["VideoInput"], **kwargs) -> dict[str, list[list["ImageObject"]]]:
        ...
        for video in videos:
            ...
            frames = self._decode_sampled_video_frames(container, video_stream, sample_indices)
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                if num_image_tokens >= len(image_grid_thw):
                    # Text-only corpora can contain literal HTML-like strings
                    # such as "<image>" or "<video>". Escape unmatched markers
                    # so they are treated as plain text instead of MM slots.
            ...
            while VIDEO_PLACEHOLDER in content:
                if num_video_tokens >= len(video_grid_thw):
                    content = content.replace(VIDEO_PLACEHOLDER, "＜video＞")
                    break
...
@dataclass
class OpenPanguVLPlugin(Qwen2VLPlugin):
    vision_start_token: str = "[unused18]"
    vision_end_token: str = "[unused20]"
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                if num_image_tokens >= len(image_grid_thw):
                    # Some text-only corpora contain the literal string "<image>" as
                    # plain text. Once all real images are consumed, escape the rest
                    # so they are treated as normal text instead of multimodal slots.
                    content = content.replace(IMAGE_PLACEHOLDER, "＜image＞")
                    break

                image_seqlen = image_grid_thw[num_image_tokens].prod() // merge_length if self.expand_mm_tokens else 1
                image_text = self.vision_start_token + (self.image_token * image_seqlen) + self.vision_end_token
                content = content.replace(IMAGE_PLACEHOLDER, image_text, 1)
                num_image_tokens += 1
            ...
            while VIDEO_PLACEHOLDER in content:
                ...
                video_seqlen = video_grid_thw[num_video_tokens].prod() // merge_length if self.expand_mm_tokens else 1
                video_text = self.vision_start_token + (self.video_token * video_seqlen) + self.vision_end_token
                content = content.replace(VIDEO_PLACEHOLDER, video_text, 1)
                num_video_tokens += 1
...
class Glm4VPlugin(Qwen2VLPlugin):
    image_start_token: str = "<|begin_of_image|>"
    image_end_token: str = "<|end_of_image|>"
    video_start_token: str = "<|begin_of_video|>"
    video_end_token: str = "<|end_of_video|>"
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                ...
                image_seqlen = image_grid_thw[num_image_tokens].prod() // merge_length if self.expand_mm_tokens else 1
                image_text = self.image_start_token + (self.image_token * image_seqlen) + self.image_end_token
                content = content.replace(IMAGE_PLACEHOLDER, image_text, 1)
                num_image_tokens += 1
            ...
            while VIDEO_PLACEHOLDER in content:
                ...
                video_seqlen = video_grid_thw[num_video_tokens].prod() // merge_length if self.expand_mm_tokens else 1
                video_text = self.video_start_token + (self.video_token * video_seqlen) + self.video_end_token
                content = content.replace(VIDEO_PLACEHOLDER, video_text, 1)
                num_video_tokens += 1
...
class Qwen2OmniPlugin(Qwen2VLPlugin):
    ...
    def _get_mm_inputs(
        ...
        if len(videos) != 0:
            ...
            if video_processor is image_processor:
                mm_inputs.update(video_processor(images=None, videos=video_dict["videos"], return_tensors="pt"))
            else:
                mm_inputs.update(video_processor(video_dict["videos"], return_tensors="pt"))
            temporal_patch_size: int = getattr(video_processor, "temporal_patch_size", getattr(image_processor, "temporal_patch_size", 2))
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content and num_image_tokens < len(images):
            ...
            if not use_audio_in_video:
                while AUDIO_PLACEHOLDER in content and num_audio_tokens < len(audios):
                ...
                while VIDEO_PLACEHOLDER in content and num_video_tokens < len(videos):
...
class InternVLPlugin(BasePlugin):
    ...
    def _validate_input(
        ...
        if len(images) != 0 and self.image_token is None:
            raise ValueError("InternVL image input requires the `internvl2_5` template.")
        if len(videos) != 0:
            raise ValueError("InternVL2.5 FLUID-V training currently does not support video samples.")
        if len(audios) != 0:
            raise ValueError("InternVL2.5 does not support audio samples.")
    ...
    def _open_image(self, image: "ImageInput") -> "ImageObject":
        if isinstance(image, str):
            ...
            if image.get("bytes") is not None:
                ...
                image = Image.open(image["path"])
        elif hasattr(image, "read") and not isinstance(image, ImageObject):
            image = Image.open(image)

        if not isinstance(image, ImageObject):
            raise ValueError(f"Expect an image input, but got {type(image)}.")
    ...
    def _find_closest_aspect_ratio(
        ...
        for ratio in target_ratios:
            ...
            if ratio_diff < best_ratio_diff:
                ...
                target_area = self.image_size * self.image_size * ratio[0] * ratio[1]
                if area > 0.5 * target_area:
                    best_ratio = ratio
        return best_ratio

    def _dynamic_preprocess(self, image: "ImageObject") -> list["ImageObject"]:
        orig_width, orig_height = image.size
        aspect_ratio = orig_width / orig_height
        target_ratios = {
            (width_blocks, height_blocks)
            for total_blocks in range(1, self.max_dynamic_patch + 1)
            for width_blocks in range(1, total_blocks + 1)
            for height_blocks in range(1, total_blocks + 1)
            if 1 <= width_blocks * height_blocks <= self.max_dynamic_patch
        }
        ...
        resized = image.resize((target_width, target_height), resampling.BICUBIC)
        processed_images = []
        for block_idx in range(blocks):
            left = (block_idx % target_ratio[0]) * self.image_size
            upper = (block_idx // target_ratio[0]) * self.image_size
            processed_images.append(
                resized.crop((left, upper, left + self.image_size, upper + self.image_size))
            )

        if self.use_thumbnail and len(processed_images) != 1:
            processed_images.append(image.resize((self.image_size, self.image_size), resampling.BICUBIC))
        return processed_images

    def _image_to_tensor(self, image: "ImageObject") -> torch.Tensor:
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                ...
                patch_count = len(self._dynamic_preprocess(self._open_image(images[image_index])))
                image_tokens = "<img>" + "<IMG_CONTEXT>" * (self.num_image_tokens * patch_count) + "</img>"
                content = content.replace(IMAGE_PLACEHOLDER, image_tokens, 1)
                image_index += 1
            message["content"] = content
    ...
    def get_mm_inputs(
        ...
        pixel_values = []
        for image in images:
            patches = self._dynamic_preprocess(self._open_image(image))
            pixel_values.extend(self._image_to_tensor(patch) for patch in patches)
...
PLUGINS = {
    ...
    "openpangu_vl": OpenPanguVLPlugin,
