...
def _get_preprocessed_features(
    data_args: "DataArguments", stage: Literal["pt", "sft", "rm", "ppo", "kto"]
) -> Optional["Features"]:
    r"""Build explicit map output features to avoid `datasets` inferring multimodal columns as null."""
    ...
    if stage == "pt":
        ...
        if data_args.packing:
            features["position_ids"] = Sequence(Value("int64"))
        else:
            features["bucket_cutoff_len"] = Value("int64")
            features["effective_k"] = Value("int64")
            features["train_mode_id"] = Value("int64")
        ...
        return Features(
            {
                ...
                "attention_mask": Sequence(Value("int64")),
                "labels": Sequence(Value("int64")),
                "kl_input_ids": Sequence(Value("int64")),
                "kl_attention_mask": Sequence(Value("int64")),
...
def _get_preprocessed_dataset(
    ...
    kwargs = {}
    output_features = _get_preprocessed_features(data_args, stage)
    if output_features is not None:
        kwargs["features"] = output_features
